...
sort $1|while read line ; do grep -ioE "import [a-zA-Z0-9]+"|sed 's/import //g'|sed -e 's/^/\^/'|while read lineB;do grep -iE $lineB requirements2.txt;done;done > requirements.txt 
