...
sort $1|while read line ; do grep -ioE "import [a-zA-Z0-9]+"|sed 's/import //g'|sed -e 's/^/\^/'|while read lineB;do grep -siE --no-filename $lineB requirements2.txt 2>/dev/null;done;done > requirements1.txt 
